/* ============================================================
   SQUAT VOICE INSTRUCTIONS - 2 METER DISTANCE
   
   Contains:
   - 2m-specific voice guidance
   - Position checking for squats at 2m
   - Instruction flow for squats
============================================================ */

import { angle } from './squat-pose-detection-2meter.js';
import { startTest, warmupHeelBaselineBeforeGo } from './squat-scoring-logic-2meter.js';
...
export function getSquatInstructions2m() {
    return squatInstructions2m;
}
...
export function startSquatAssistant2m(getSmoothedLandmarks, onReturnToMenu = null) {
    ...
    speak(squatInstructions2m[0], () => (startInstruct = true));
    startInterval = setInterval(() => checkSquatSteps2m(getSmoothedLandmarks), 1000);
}

function checkSquatSteps2m(getSmoothedLandmarks) {
    ...
    const orientationOk = () => {
        const basicOk = isInCorrectSquatPosition2m(lm);
...
function nextSquatInstruction2m() {
    startInstruct = false;
    setTimeout(() => {
        if (speechCancelled) return;
        
        startStep++;
        if (startStep < squatInstructions2m.length - 1) {
            speak(squatInstructions2m[startStep], () => (startInstruct = true));
        } else {
            if (speechCancelled) return;
            waitingForGoPosition = true;
            hasWarnedPosition = false;
...
function isInCorrectSquatPosition2m(lm) {
...
export function stopSquatVoiceAssistant2m() {
    voiceStarted = false;
    startInstruct = false;
    isCaptureActive = false;
    lowerBodyStableFrames = 0;
...
export function resetSquatVoiceState2m() {
    voiceStarted = false;
    startStep = 0;
    startInstruct = false;
    isCaptureActive = false;
...
export function getSquatVoiceStarted2m() {
    return voiceStarted;
}

export function getSquatIsCaptureActive2m() {
