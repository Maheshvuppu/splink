/* ============================================================
   T-POSE VOICE INSTRUCTIONS - 1 METER DISTANCE
   
   Contains:
   - 1m-specific voice guidance
   - Position checking for t-pose at 1m
   - Instruction flow for t-pose balance test
============================================================ */

import { angle } from './t-pose-pose-detection-1meter.js';
import { startTPoseBalanceTest } from './t-pose-scoring-logic-1meter.js';
...
export function getTPoseInstructions1m() {
    return tPoseInstructions1m;
}
...
export function startTPoseAssistant1m(getSmoothedLandmarks, onReturnToMenu = null) {
    ...
    speak(tPoseInstructions1m[0], () => (startInstruct = true));
    startInterval = setInterval(() => checkTPoseSteps1m(getSmoothedLandmarks), 1000);
}

function checkTPoseSteps1m(getSmoothedLandmarks) {
    ...
    const orientationOk = () => {
        const basicOk = isInCorrectTPosePosition1m(lm);
...
function nextTPoseInstruction1m() {
    startInstruct = false;
    setTimeout(() => {
        if (speechCancelled) return;
        
        startStep++;
        if (startStep < tPoseInstructions1m.length - 1) {
            speak(tPoseInstructions1m[startStep], () => (startInstruct = true));
        } else {
            if (speechCancelled) return;
            startInstruct = true;
        }
...
function isInCorrectTPosePosition1m(lm) {
...
export function stopTPoseVoiceAssistant1m() {
    voiceStarted = false;
    startInstruct = false;
    isCaptureActive = false;
    lowerBodyStableFrames = 0;
...
export function resetTPoseVoiceState1m() {
    voiceStarted = false;
    startStep = 0;
    startInstruct = false;
    isCaptureActive = false;
...
export function getTPoseVoiceStarted1m() {
    return voiceStarted;
}

export function getTPoseIsCaptureActive1m() {
    return isCaptureActive;
}

export { isInCorrectTPosePosition1m as isInCorrectTPosePosition, startTPoseBalanceTest };
