/* ============================================================
   SQUAT VOICE INSTRUCTIONS - 1 METER DISTANCE
   
   Contains:
   - 1m-specific voice guidance
   - Position checking for squats at 1m
   - Instruction flow for squats
============================================================ */

import { angle } from './squat-pose-detection-1meter.js';
import { startTest, warmupHeelBaselineBeforeGo } from './squat-scoring-logic-1meter.js';
...
const squatInstructions1m = [
    "Show your full body to the camera",
    "Rotate left, showing your right shoulder.",
    "Raise your hands straight up.",
    "Perform 5 strict squats. You have 12 seconds."
];

export function getSquatInstructions1m() {
    return squatInstructions1m;
}
...
export function startSquatAssistant1m(getSmoothedLandmarks, onReturnToMenu = null) {
    ...
    speak(squatInstructions1m[0], () => (startInstruct = true));
    startInterval = setInterval(() => checkSquatSteps1m(getSmoothedLandmarks), 1000);
}

function checkSquatSteps1m(getSmoothedLandmarks) {
    ...
    const k = Math.min(leftAngle, rightAngle);
    ...
    if (waitingForGoPosition) {
        ...
        if (inCorrectPosition) {
            ...
            if (needsHandsRecheck) {
                ...
                if (!handsCurrentlyUp) {
                    // Check if only one hand is up and prompt for the other
                    const now = Date.now();
...
function nextSquatInstruction1m() {
    startInstruct = false;
    setTimeout(() => {
        if (speechCancelled) return;
        
        startStep++;
        if (startStep < squatInstructions1m.length - 1) {
            speak(squatInstructions1m[startStep], () => (startInstruct = true));
        } else {
            if (speechCancelled) return;
            waitingForGoPosition = true;
            hasWarnedPosition = false;
...
function isInCorrectSquatPosition1m(lm) {
...
export function stopSquatVoiceAssistant1m() {
    voiceStarted = false;
    startInstruct = false;
    isCaptureActive = false;
    lowerBodyStableFrames = 0;
...
export function resetSquatVoiceState1m() {
    voiceStarted = false;
    startStep = 0;
    startInstruct = false;
    isCaptureActive = false;
...
export function getSquatVoiceStarted1m() {
    return voiceStarted;
}

export function getSquatIsCaptureActive1m() {
