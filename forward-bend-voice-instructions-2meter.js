/* ============================================================
   FORWARD BEND VOICE INSTRUCTIONS - 2 METER DISTANCE
   
   Contains:
   - 2m-specific voice guidance
   - Position checking for forward bend at 2m
   - Instruction flow for forward bend
============================================================ */

import { angle } from './forward-bend-pose-detection-2meter.js';
import { startForwardBendTest } from './forward-bend-scoring-logic-2meter.js';
...
const forwardBendInstructions2m = [
...
export function getForwardBendInstructions2m() {
    return forwardBendInstructions2m;
}
...
export function startForwardBendAssistant2m(getSmoothedLandmarks, onReturnToMenu = null) {
    ...
    speak(forwardBendInstructions2m[0], () => (startInstruct = true));
    startInterval = setInterval(() => checkForwardBendSteps2m(getSmoothedLandmarks), 1000);
}

function checkForwardBendSteps2m(getSmoothedLandmarks) {
    ...
    const orientationOk = () => {
        const basicOk = isInCorrectForwardBendPosition2m(lm);
    ...
    if (waitingForGoPosition) {
        ...
        if (inCorrectPosition) {
            ...
            speak(forwardBendInstructions2m[3], () => {
...
function nextForwardBendInstruction2m() {
    startInstruct = false;
    setTimeout(() => {
        if (speechCancelled) return;
        
        startStep++;
        if (startStep < forwardBendInstructions2m.length - 1) {
            speak(forwardBendInstructions2m[startStep], () => (startInstruct = true));
        } else {
            if (speechCancelled) return;
            waitingForGoPosition = true;
            hasWarnedPosition = false;
...
function isInCorrectForwardBendPosition2m(lm) {
...
export function stopForwardBendVoiceAssistant2m() {
    voiceStarted = false;
    startInstruct = false;
    isCaptureActive = false;
    lowerBodyStableFrames = 0;
...
export function resetForwardBendVoiceState2m() {
    voiceStarted = false;
    startStep = 0;
    startInstruct = false;
    isCaptureActive = false;
...
export function getForwardBendVoiceStarted2m() {
    return voiceStarted;
}

export function getForwardBendIsCaptureActive2m() {
    return isCaptureActive;
}

export { isInCorrectForwardBendPosition2m as isInCorrectForwardBendPosition };
