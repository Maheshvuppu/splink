/* ============================================================
   HIGH KNEE VOICE INSTRUCTIONS - 2 METER DISTANCE
   
   Contains:
   - 2m-specific voice guidance
   - Position checking for high knee at 2m
   - Instruction flow for high knee
============================================================ */

import { angle } from './high-knee-pose-detection-2meter.js';
import { startHighKneeMarchTest } from './high-knee-scoring-logic-2meter.js';
...
const highKneeInstructions2m = [
    "Show your full body to the camera",
    "Face the camera directly.",
    "March in place with high knees. You have 10 seconds."
];

export function getHighKneeInstructions2m() {
    return highKneeInstructions2m;
}
...
// 2m early-march detection thresholds (aligned with scoring logic)
const EARLY_HIGHKNEE_UP_BEND = 20;
const EARLY_HIGHKNEE_DOWN_BEND = 12;
const EARLY_HIGHKNEE_ANKLE_DIFF_THRESHOLD = 0.03;

export function startHighKneeAssistant2m(getSmoothedLandmarks, onReturnToMenu = null) {
    ...
    speak(highKneeInstructions2m[0], () => (startInstruct = true));
    startInterval = setInterval(() => checkHighKneeSteps2m(getSmoothedLandmarks), 1000);
}

function checkHighKneeSteps2m(getSmoothedLandmarks) {
    ...
    if (waitingForGoPosition) {
        const inCorrectPosition = isInCorrectHighKneePosition2m(lm);
        if (inCorrectPosition) {
            ...
            if (hasWarnedPosition || needsInstructionRepeatBeforeGo) {
                speak(highKneeInstructions2m[2], () => {
    ...
    if (startStep > 0) {
        if (lm[11] && lm[12] && lm[23] && lm[24]) {
            const isInCorrectOrientation = isInCorrectHighKneePosition2m(lm);
    ...
    switch (startStep) {
        case 0:
            if (k > 160) {
                lowerBodyStableFrames = 0;
                nextHighKneeInstruction2m();
            }
            break;

        case 1:
            if (isInCorrectHighKneePosition2m(lm)) {
...
function nextHighKneeInstruction2m() {
    startInstruct = false;
    setTimeout(() => {
        if (speechCancelled) return;
        
        startStep++;
        if (startStep < highKneeInstructions2m.length - 1) {
            speak(highKneeInstructions2m[startStep], () => (startInstruct = true));
        } else {
            if (speechCancelled) return;
            startInstruct = true;
        }
...
function isInCorrectHighKneePosition2m(lm) {
...
export function stopHighKneeVoiceAssistant2m() {
    voiceStarted = false;
    startInstruct = false;
    isCaptureActive = false;
    lowerBodyStableFrames = 0;
...
export function resetHighKneeVoiceState2m() {
    voiceStarted = false;
    startStep = 0;
    startInstruct = false;
    isCaptureActive = false;
...
export function getHighKneeVoiceStarted2m() {
    return voiceStarted;
}

export function getHighKneeIsCaptureActive2m() {
    return isCaptureActive;
}

export { isInCorrectHighKneePosition2m as isInCorrectHighKneePosition };
